...
First, we will need a Linux operating system installed on your computer. We will be using Pop!_OS as the operating system running the virtualization software, called the Host operating system. Then, we will use Ubuntu as the "Guest" operating system, which runs inside the VM.
...
**NOTE:** _For this guide, we used GNOME-based Linux distributions, so the VMs UI closely matched Pop!\_OS and Ubuntu. Accessing shared folders with other desktop environments is possible, but you will need to refer to their documentation for the details._
