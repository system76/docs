...
- Then install ArSandbox via the [terminal](../terminal-basics/index.mdx)
...
- Mirror your computer's display to the sandbox by pressing `Super` <kbd><Icon name="simple-icons:popos" /></kbd> and `p` at the same time until `Mirror` is selected. When released the sandbox should mirror your computer's display.
