...
Please refer to this [support article](../open-firmware-smmstore/index.md). If you see the message below then you can reboot the system using `sudo systemctl reboot` to proceed with the update, or use `sudo system76-firmware-cli unschedule` to cancel the update.
...
* [Live Disk creation on Pop!_OS](../pop-live-disk/index.md)
* [Live Disk creation on Other OS's](../live-disk/index.md)

After creating the live disk, [access the boot menu](../boot-menu/index.md) and select it from the list of options. Once the desktop appears, open a terminal (<kbd><Icon name="simple-icons:popos" /></kbd> + <kbd>T</kbd>) and use this command to list the partitions on your system:
...
If you would like to transition from proprietary firmware to System76 Open Firmware (on a supported model), please view [this article](../transition-firmware/index.md).
